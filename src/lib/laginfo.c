...
int laginfo_add_lags_list (int n, Laginfo *linfo)
{
    int **llists;
    int err = 0;
...
int laginfo_expand_reflist (int n, int v, Laginfo *linfo)
{
    int *reflist;
    int l0, err = 0;
...
int expand_srclist (int srcv, int lpos, Laginfo *linfo,
               const int *cmdlist)
{
    int i, n = lpos + 1;
    int err = 0;
...
const char *lag_sign_str (int lag)
{
    if (lag > 0) return "-";
    if (lag < 0) return "+";
    else return "";
...
int print_var_lags (const int *laglist, int gotsep,
               PRN *prn)
{
    char tmp[32];
    int lag, lsign;
